...
const navItems = [
  ...
  { icon: <UsersIcon />, label: 'Utilisateurs', to: '#' },
  { icon: <ClipboardIcon />, label: 'Missions', to: '#' },
  { icon: <SettingsIcon />, label: 'Réglages', to: '#' },
]
...
function AdminLayout({ active, title, subtitle, headerActions, validationsCount = 0, children }) {
        ...
        <div className="flex items-center justify-between mb-8 gap-4">
          ...
          <div className="flex items-center gap-4 shrink-0">
            <button className="relative text-gray-500 hover:text-babi-dark transition-colors">
              <BellIcon />
              {validationsCount > 0 && <span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-rose-500 rounded-full"></span>}
            </button>
